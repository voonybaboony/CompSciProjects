...
def eval_all(expressions, env):
    ...
    if not expressions:
        ...
        if not expressions.second:
            return scheme_eval(expressions.first, env, True) #Changed
        else:
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        while formals:
            ...
            if (formals and not vals) or (vals and not formals):
                raise SchemeError
...
def do_if_form(expressions, env):
    ...
    if scheme_truep(scheme_eval(expressions.first, env)):
        return scheme_eval(expressions.second.first, env, True) #Changed
    elif len(expressions) == 3:
        return scheme_eval(expressions.second.second.first, env, True) #Changed
