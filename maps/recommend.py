...
def find_closest(location, centroids):
    ...
    """
    # BEGIN Question 3
    """closest_pair = centroids[0]
    for x in centroids:
        dist = distance(x, location)
        prev_distance = distance(closest_pair, location)
...
def find_centroid(cluster):
    ...
    for i in cluster:
        x_list.append(restaurant_location(i)[0])
        y_list.append(restaurant_location(i)[1])

    return [mean(x_list), mean(y_list)]
...
def best_predictor(user, restaurants, feature_fns):
    ...
    for f in feature_fns:
        pred, r_squared = find_predictor(user, reviewed, f)
        predictors.append(pred)
        rs.append(r_squared)

    lis = zip(predictors, rs)
