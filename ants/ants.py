...
class Bee(Insect):
    ...
    def action(self, colony):
        ...
        elif self.armor > 0 and (destination is not None or destination is not colony.hive):
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        nearest_bee_place = None
        current_place = self.place
        count_range = 0
        while nearest_bee_place is None and current_place.entrance is not hive:
            if count_range >= self.min_range and count_range <= self.max_range and len(current_place.bees) is not 0:
                nearest_bee_place = current_place
            else:
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.imposter:
            ...
            ScubaThrower.action(self, colony) #throws leaf at bee
            current_place = self.place.exit
            while current_place is not None: #buffs every ant behind her once
                subject = current_place.ant
                if subject is not None: #if current_place has an ant
                    if subject.is_container: #if the ant is a container
                        if subject.is_buffed is False: #if it has not been buffed already
                            subject.damage *= 2
                            subject.is_buffed = True
                        if subject.contained_ant is not None: #if the container contains another ant
                            if subject.contained_ant.is_buffed is False: #if it has not been buffed already
                                subject.contained_ant.damage *= 2
                                subject.contained_ant.is_buffed = True
                    else:                                          #if the ant is not a container
                        if subject.is_buffed is False:
                            subject.damage *= 2
                            subject.is_buffed = True
...
def apply_effect(effect, bee, duration):
    ...
    mod_action = effect(bee.action, bee)
